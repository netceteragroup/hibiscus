/* #CHANGED check these tags to find changes to the original file */

/* #CHANGED paths updates to link to the corresponding files in the node_modules-folder */
...
var diveSync = require('diveSync'),
    ...
    pm = require('./../../../node_modules/patternlab-node/core/lib/plugin_manager'),
    fs = require('fs-extra'),
    plutils = require('./../../../node_modules/patternlab-node/core/lib/utilities');
...
function initializePlugins(patternlab) {
  ...
  var plugin_manager = new pm(patternlab.config, path.resolve(__dirname, './../../../node_modules/patternlab-node/patternlab-config.json'));
...
var patternlab_engine = function (config) {
  ...
  var JSON5 = require('json5'),
      pa = require('./../../../node_modules/patternlab-node/core/lib/pattern_assembler'),
      pe = require('./../../../node_modules/patternlab-node/core/lib/pattern_exporter'),
      lh = require('./../../../node_modules/patternlab-node/core/lib/lineage_hunter'),
      ui = require('./../../../node_modules/patternlab-node/core/lib/ui_builder'),
      sm = require('./../../../node_modules/patternlab-node/core/lib/starterkit_manager'),
      Pattern = require('./../../../node_modules/patternlab-node/core/lib/object_factory').Pattern,
      patternlab = {};
  ...
  patternlab.package = fs.readJSONSync(path.resolve(__dirname, './../../../node_modules/patternlab-node/package.json'));
  patternlab.config = config || fs.readJSONSync(path.resolve(__dirname, './../../../node_modules/patternlab-node/patternlab-config.json'));
  ...
  function writePatternFiles(headHTML, pattern, footerHTML, magnoliaMode) {
    ...
    if (!magnoliaMode) {
      ...
      //console.log('renderMagnolia')
      const makePath = type => path.join(paths.temp.patterns, pattern.getPatternLink(patternlab, type));
      const outputFilesMagnoliaMode = [
        {path: makePath('markupOnly'), content: formatters.markupOnly(pattern.patternPartialCode, pattern)}
      ].concat(eng.addOutputFiles ? eng.addOutputFiles(paths, patternlab) : []);
        //write the compiled template to the public patterns directory
        outputFilesMagnoliaMode.forEach(outFile => fs.outputFileSync(outFile.path.replace('.markup-only.html', '.ftl'), outFile.content));
    }
  }

  /* #CHANGED added magnoliaMode-parameter */
  function buildPatterns(deletePatternDir, magnoliaMode) {
    ...
    patternlab.patterns.forEach(function (pattern) {
      ...
      //write the compiled template to the public patterns directory
      writePatternFiles(headHTML, pattern, footerHTML, magnoliaMode);
  ...
  return {
    ...
    build: function (callback, deletePatternDir) {
      ...
      buildPatterns(deletePatternDir, false);
    ...
    /* #CHANGED added magnoliaMode-parameter, if true it will render the styleguide specific entries*/
    patternsonly: function (callback, deletePatternDir) {
      ...
      patternlab.isBusy = true;
      /* #CHANGED added magnoliaMode-parameter, if true it will render the styleguide specific entries*/
      buildPatterns(deletePatternDir, true);
      printDebug();
      patternlab.isBusy = false;
      callback();
    },
