...
var patternlab_engine = function (config) {
  ...
  var path = require('path'),
    ...
    of = require('./../../../node_modules/patternlab-node/core/lib/object_factory'),
    pa = require('./../../../node_modules/patternlab-node/core/lib/pattern_assembler'),
    mh = require('./../../../node_modules/patternlab-node/core/lib/media_hunter'),
  ...
  function buildPatterns(deletePatternDir, magnoliaMode) {
    ...
    patternlab.patterns.forEach(function (pattern) {
      ...
      if (patternlab.userFoot) {
        userFooter = patternlab.userFoot.extendedTemplate.replace('{% pattern-lab-foot %}', patternlab.footerPattern + patternlab.footer);
      ...
      /* #CHANGED added method to render ftl templates and created if-else to run corresponding renderer*/

      if (magnoliaMode) {
      //write the compiled template to the public patterns directory
      fs.outputFileSync(paths.temp + pattern.patternLink.replace('.html', '.ftl'), pattern.patternPartial);
  ...
  return {
    ...
    build: function (deletePatternDir) {
      /* #CHANGED added magnoliaMode-parameter, if false it will render the ftl-specific entries*/
